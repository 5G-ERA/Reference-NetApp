...
class MMDetector(ImageDetector):
    ...
    def __init__(self, logger, name,):
        ...
        if not self.path_to_mmdet:
            raise ImageDetectorInitializationFailed(f"Failed to load mmdet module, env variable NETAPP_MMDET_PATH not set")
        if not os.path.exists(self.path_to_mmdet):
            raise ImageDetectorInitializationFailed(f"Failed to load mmdet module, path {self.path_to_mmdet} does not exist")
    ...
    def process_image(self, frame):
        """
        Detects the objects of selected classes in the passed frame and returns all detections.
        ...
        if frame is not None:
            ...
            return convert_mmdet_result(result, merged_data=True, with_mask=MODEL_VARIANTS[self.model_variant]['with_masks'])
        else:
