...
class FaceDetector(ImageDetector, ABC):
    ...
    def process_image(self, frame: np.array) -> BasicDetectorResultType:
        ...
        Returns:
            BasicDetectorResultType: The list of detected faces, with bounding box (x1, y1, x2, y2,
            top-left bottom-right corners), score (0..1), class_id (equals to 1 for faces) and class_name ("face").
        """
    ...
    def process_images(self, frames: List[np.array]) -> List[BasicDetectorResultType]:
        ...
        Returns:
            List[BasicDetectorResultType]: List with results for each of the batched frames. Each item is a list of
                detected faces, with bounding box (x1, y1, x2, y2, top-left bottom-right corners), score (0..1),
